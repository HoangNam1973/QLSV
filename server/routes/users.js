...
router.post('/update', auth, async (req, res) => {
  ...
  const { username, password, phone, address, sex, dateBirth } = req.body;
  try {
    if (!id) {
...
// Endpoint khởi tạo admin (chỉ dùng 1 lần, sau đó nên xóa hoặc bảo vệ)
router.post('/init-admin', async (req, res) => {
  try {
    const existing = await User.findOne({ account: 'admin@domain.com' });
    if (existing) return res.json({ status: true, message: 'Admin already exists' });
    ...
    await admin.save();
    res.json({ status: true, message: 'Admin created', account: 'admin@domain.com', password: 'admin123' });
  } catch (err) {
